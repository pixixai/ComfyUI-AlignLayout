...
// 定义"恢复默认"的特殊设置项
const resetSetting = {
    id: "AlignLayout.ReplaceNode.ResetAll",
    name: "Restore Default Settings",
    type: "boolean",
    defaultValue: false,
    category: ["AlignLayout", "Replace Node", "RestoreDefaultSettings"], 
    tooltip: "⚠️ WARNING: Click to RESET all Replace Node settings. The switch will auto-off after execution.",
    onChange: async (newVal, oldVal) => {
        if (newVal === true) {
            const confirmed = confirm("Are you sure you want to reset all Replace Node settings to defaults?");

            if (confirmed) {
                for (const setting of replaceNodeSettings) {
                    try {
            ...
            setTimeout(() => {
                app.extensionManager.setting.set("AlignLayout.ReplaceNode.ResetAll", false);
            }, 100);
        }
    }
};
...
app.registerExtension({
    ...
    // 将普通设置和重置按钮合并
    settings: [...replaceNodeSettings, resetSetting]
});