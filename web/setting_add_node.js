...
// 定义"恢复默认"的特殊设置项
const resetSetting = {
    id: "AddNodeMenu.ResetAll",
    name: "Restore Default Settings",
    type: "boolean",
    defaultValue: false,
    category: ["AlignLayout", "AddNodeMenu", "RestoreDefaultSettings"], 
    tooltip: "⚠️ WARNING: Click to RESET all Add Node Menu settings. The switch will auto-off after execution.",
    onChange: async (newVal, oldVal) => {
        // 只有当用户将其打开 (true) 时才触发
        if (newVal === true) {
            // 1. 弹出确认框
            const confirmed = confirm("Are you sure you want to reset all Add Node Menu settings to defaults?");

            if (confirmed) {
                // 2. 遍历数组写入默认值
                for (const setting of addNodeMenuSettings) {
                    try {
            ...
            // 3. 自动拨回 false
            setTimeout(() => {
                app.extensionManager.setting.set("AddNodeMenu.ResetAll", false);
            }, 100);
        }
    }
};
...
app.registerExtension({
    ...
    // 将普通设置和重置按钮合并
    settings: [...addNodeMenuSettings, resetSetting]
});